...
您可以通过两种方式从子账户提取资金：
1.  **法币支付：** 将货币余额转换为当地货币（如 NGN、BRL、EUR）并发送到银行账户。
...
从子账户向已创建的收款人发送资金。系统将从子账户中扣除相应的货币余额。
...
> **注意：** `amount` 字段指定的是**目标货币**的金额（如 NGN）。等值余额将从子账户的货币持有中扣除。
...
您也可以将余额直接作为稳定币（USDC 或 USDT）提取到外部钱包地址。
...
3.  **余额不足：** 确保子账户有足够的相关货币余额来支付提款金额**加上费用**。
4.  **未验证的子账户：** 仅允许已通过验证的子账户进行提款。
